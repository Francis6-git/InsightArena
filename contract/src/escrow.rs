...
mod escrow_tests {
    ...
    use super::{assert_escrow_solvent, get_contract_balance, lock_stake, release_payout};
    ...
    fn test_assert_escrow_solvent_when_balance_covers_unclaimed_stakes() {
        ...
        env.as_contract(&client.address, || {
            ...
            env.storage().persistent().set(&DataKey::PredictorList(1), &predictors_one);

            let mut predictors_two = Vec::new(&env);
            predictors_two.push_back(predictor_b.clone());
            env.storage().persistent().set(&DataKey::PredictorList(2), &predictors_two);

            env.storage().persistent().set(&DataKey::MarketCount, &2_u64);
            env.storage().persistent().set(
                &DataKey::Prediction(1, predictor_a.clone()),
                &Prediction::new(1, predictor_a.clone(), soroban_sdk::symbol_short!("yes"), 20_000_000, env.ledger().timestamp()),
            );
            env.storage().persistent().set(
                &DataKey::Prediction(2, predictor_b.clone()),
                &Prediction::new(2, predictor_b.clone(), soroban_sdk::symbol_short!("no"), 30_000_000, env.ledger().timestamp()),
            );
        });
    ...
    fn test_assert_escrow_solvent_when_balance_is_short() {
        ...
        env.as_contract(&client.address, || {
            ...
            env.storage().persistent().set(&DataKey::PredictorList(1), &predictors);
            env.storage().persistent().set(&DataKey::MarketCount, &1_u64);
            env.storage().persistent().set(
                &DataKey::Prediction(1, predictor.clone()),
                &Prediction::new(1, predictor.clone(), soroban_sdk::symbol_short!("yes"), 20_000_000, env.ledger().timestamp()),
            );
        });
